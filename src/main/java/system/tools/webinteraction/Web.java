...
import system.tools.configuration.Log;
...
public class Web {
    ...
    public static void moveMouseToElement(WebElement element, WebDriver driver) {
        Actions action = new Actions(driver);
        action.moveToElement(element).build().perform();
    }

    public static void rolarScrollIntoView(WebDriver driver, WebElement elem) {
        JavascriptExecutor js = (JavascriptExecutor) driver;
        js.executeScript("arguments[0].scrollIntoView();", elem);
    }

    public static void rolarScroll(WebDriver driver, int pixels, boolean praBaixo) {
    ...
    public static void rolarScrollComElem(WebDriver driver, boolean praBaixo, WebElement elem) {
        JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
        jsExecutor.executeScript("arguments[0].scrollIntoView(" + praBaixo + ");", elem);
    }

    public static void rolarScrollExtraComElem(WebDriver driver, boolean praBaixo, WebElement elem) {
        JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
        jsExecutor.executeScript("arguments[0].scrollIntoView(" + praBaixo + ");", elem);
        rolarScroll(driver, 120, true);
    }
    ...
    public static void trocarAbaUrlIgualParametro(WebDriver driver, String urlEsperada) {
        Set<String> janelas = driver.getWindowHandles();
        String urlAux = "";
        for (String janela : janelas) {
            driver.switchTo().window(janela);
    ...
    public static String montarXpath(WebDriver driver, String xpathBaseTabela, String elemReferencia, int colunaElemDesejado) {
        encontrarLinhaDesejada(driver, xpathBaseTabela, elemReferencia);
        String xpathMontado = xpathElemAtual.substring(0, xpathElemAtual.lastIndexOf('[') + 1) + colunaElemDesejado + "]";
        return xpathMontado;
    }

    public static void encontrarLinhaDesejada(WebDriver driver, String xpathBaseTabela, String elemReferencia) {
    ...
    public static void retryToClick(WebElement element) {
    ...
    public static void clickClearAndSendKeys(WebElement elem, String valor) {
        elem.click();
        elem.clear();
        elem.sendKeys(valor);
    }

    public static void clickAndClearWithKeysChord(WebElement elem) {
        elem.click();
        elem.sendKeys(Keys.chord(Keys.CONTROL, "a", Keys.DELETE));
    }

    public static void clickClearAndSendKeysWithKeysChord(WebElement elem, String valor) {
        clickAndClearWithKeysChord(elem);
        elem.sendKeys(valor);
    }

    public static void clickClearSendKeysAndTabWithKeysChord(WebElement elem, String valor) {
        clickAndClearWithKeysChord(elem);
        elem.sendKeys(valor);
...
//    }
//
    public static void selecionarItemDaCombo(WebElement elemSelecao, String opcaoDesejada) throws Exception {
...
//    }
//
    public static void realizarUploadArquivo(String caminhoArquivo, int tempoEsperaJanela) throws InterruptedException, AWTException {
        StringSelection ss = new StringSelection(caminhoArquivo);
        Toolkit.getDefaultToolkit().getSystemClipboard().setContents(ss, ss);
        Thread.sleep(tempoEsperaJanela);
